...
//		printf("%f to restart!\n",(1+Width-time*.1f));
//view->Set(Rect(0,0,1,1));
	break;
	case 2:
//printf("Tick!\n");
		time-=start_t;
//if(time>out) return false;
		view->Set(Rect(row,1-time*.1f,row+Width,1-time*.1f+.04f*15.0f));
		if((1-time*.1f)<0) start_t+=time; //return false;
//		printf("%f to restart!\n",(1+Width-time*.1f));
//view->Set(Rect(0,0,1,1));
	case 3:
		time-=start_t;
...
*/		"Systems Interconnection). AF_UNIX and AF_INET are the most important address families. The general form of\r"
		".73,1,2,1,1,1,"
		" 1. Navn  1\n"
		" 2. Navn  2\n"
		" 3. Navn  3\n"
...
//		".91,2,3,1,1,1,mer svada text\n"
	...
	for(int n=0; n<len; n++) if(testText[n]=='\r') testText[n]='\0';
	text = testText;
	textptr = text;
#else
	text = 0; //testText;
